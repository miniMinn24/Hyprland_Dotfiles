...
TEMP_FILE="/tmp/volume_widget_timer"
LOCK_FILE="/tmp/volume_widget_lock"
CFG="$HOME/.config/eww"
...
show_widget() {
    eww -c "$CFG" open volume_bar  # Replace with the actual command to show your widget
...
hide_widget() {
    if eww -c "$CFG" list-windows ; then
        eww -c "$CFG" close volume_bar # Replace with the actual command to hide your widget
    fi
}
